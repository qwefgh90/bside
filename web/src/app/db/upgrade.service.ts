...
export class UpgradeService {
  ...
  async migrateFrom0To1(ev: IDBVersionChangeEvent) {
    ...
    var objectStore = db.createObjectStore(allVersionInfo[1].storeName, { keyPath: allVersionInfo[1].keyPropertyName, autoIncrement: true });
    ...
    objectStore.transaction.oncomplete = (event) => {
      let tran = db.transaction(allVersionInfo[1].storeName, "readwrite");
      let store = tran.objectStore(allVersionInfo[1].storeName);
